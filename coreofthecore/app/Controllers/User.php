...
class User extends BaseController
{
    ...
    //     $uploadPath = '/home/nativese/public_html/cloud';
    //     $newName = $file->getRandomName();
    //     $file->move($uploadPath, $newName);
    //     return redirect()->route('admin/list_files')->with('message', 'File uploaded successfully.');
    ...
    public function upload()
{
    ...
    $validation = $this->validate([
        'file' => [
            'uploaded[file]',
            'max_size[file,10240]', 
            'ext_in[file,jpg,jpeg,docx,pdf,so,ehi]'
        ]
    ]);
    ...
    if ($file->isValid() && !$file->hasMoved()) {
        ...
        if ($file->move($uploadPath, $newName)) {
            ...
            return redirect()->back()->with('error', 'Failed to move the file.');
        }
    } else {
        return redirect()->back()->with('error', 'Invalid file or file has been moved.');
    }
}
    ...
    {
        // Mendapatkan daftar file di folder cloud
        $path = '/home/nativese/public_html/cloud';
        $files = array_diff(scandir($path), ['.', '..']); // Hapus tanda '.' dan '..'

        // Kirim daftar file ke view
        return view('Admin/list_files', ['files' => $files]);
    }

    public function delete_file($fileName)
    {
        // Path ke file yang akan dihapus
        $filePath = '/home/nativese/public_html/cloud/' . $fileName;

        // Hapus file jika ada
        if (file_exists($filePath)) {
            unlink($filePath);
            return redirect()->to('/admin/list_files')->with('message', 'File deleted successfully.');
        } else {
            return redirect()->to('/admin/list_files')->with('message', 'File not found.');
        }
    }
